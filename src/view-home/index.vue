            ...
            partList.forEach(part_item => {
                ...
                departmentList.forEach(depart_item => {
                    ...
                    // 第二大种情况，两个区域没有得边没有重合得部分，只能在某一角
                        ...
                        // 4个纵坐标也是一样的
                    const flag2_X = Math.max(part_item.coordinate.width, depart_item.coordinate.width) < (MAX_X - MIN_X) < (part_item.coordinate.width + depart_item.coordinate.width)
                    const flag2_Y = Math.max(part_item.coordinate.height, depart_item.coordinate.height) < (MAX_Y - MIN_Y) < (part_item.coordinate.height + depart_item.coordinate.height)
