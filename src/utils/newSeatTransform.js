...
function setTransform(element,scaleNum){
    ...
    switch(element.parentNode.className.slice(element.parentNode.className.lastIndexOf('-') + 1)){
        case 'west':
            styleString = `scale(${scaleNum}) rotateZ(0deg)`
        break
        case 'east':
            styleString = `scale(${scaleNum}) rotateZ(180deg)`
        break
        case 'south':
            styleString = `scale(${scaleNum}) rotateZ(270deg)`
        break
        case 'north':
            styleString = `scale(${scaleNum}) rotateZ(90deg)`
        break
    }
    return styleString
}
